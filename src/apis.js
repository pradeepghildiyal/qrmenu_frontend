...
export function addPlace(data, token) {
  return request("/api/places/", {data:data,token:token, method: "POST"});
}
...
export function uploadImage(image) {
  ...
  return fetch("https://api.cloudinary.com/v1_1/pradeepghildiyal/image/upload", {
    method: "POST",
    body: formData
  }).then((response) => {
      return response.json()
...
export function updateMenuItem(id, data, token) {
  return request(`/api/menu_items/${id}`, {
    data, token, method: "PATCH"
  });
}
