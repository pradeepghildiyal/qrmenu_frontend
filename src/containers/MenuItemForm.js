...
import { Button, Form, Popover, Overlay, FormGroup } from 'react-bootstrap'
...
const MenuItemForm = ({place, onDone, item={}}) => {
  ...
  const [isAvailable, setIsAvailable] = useState(
    item.isAvailable === undefined ? true : !!item.is_available
  )
  ...
  const onUpdateMenuItem = async () => {
    ...
    if (json) {
      console.log(json);
      toast(`Menu Item ${json.name} updated.`, { type: "success" });
      setCategory("");
      setName("");
      setPrice(0);
      setDescription("");
